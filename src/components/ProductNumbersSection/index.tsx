...
export const ProductNumbersSection = () => {
    ...
    const setRangeBackground = (range: string) => {
        return `linear-gradient(90deg, hsl(176, 50%, 47%) ${(parseInt(range) / 100000) * 100}%, #ededed ${(parseInt(range) / 100000) * 100}%)`
    }
    ...
    return (
        <Container>
            <Content>
                ...
                <input style={{ background: `${setRangeBackground(range)}` }} onChange={e => setRange(e.target.value)} type="range" min="0" max="100000" value={range} className="value-range" />
            </Content>
        </Container>
    )
}